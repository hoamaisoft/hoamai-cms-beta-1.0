$(document).ready(function(){
	...
	var build_table = function (data,status){
		$('.taxonomy_table .taxonomy_table_content').html('<tr><td colspan="4">Đang tải ...</td></tr>');
		var obj = jQuery.parseJSON(data);
		var html_content = [];
		...
		$.each( taxonomy, function( number , value ) {
			...
			var slug = value.slug;
			if(status == 'public'){
				var line = '<td><input name="taxonomy_ids[]" value="'+id+'" type="checkbox" class="checkall_item_1"></td><td><a href="?run=taxonomy.php&action=edit&id='+id+'">'+name+'</a></td><td>'+slug+'</td><td class="td_action td_action_'+id+'"><button type="button" data-id="'+id+'" class="quick_edit_taxonomy_button btn btn-default btn-xs">Sửa nhanh</button><a class="btn btn-default btn-xs" href="?run=taxonomy.php&action=edit&id='+id+'">Sửa</a><button type="button" data-id="'+id+'" class="quick_delete_taxonomy_button btn btn-danger btn-xs">Xóa</button></td>';
			}
			if(status == 'draft'){
				var line = '<td><input name="taxonomy_ids[]" value="'+id+'" type="checkbox" class="checkall_item_1"></td><td><a href="?run=taxonomy.php&action=edit&id='+id+'">'+name+'</a></td><td>'+slug+'</td><td class="td_action td_action_'+id+'"><button type="button" data-id="'+id+'" class="quick_edit_taxonomy_button btn btn-default btn-xs">Sửa nhanh</button><a class="btn btn-default btn-xs" href="?run=taxonomy.php&action=edit&id='+id+'">Sửa</a><button type="button" data-id="'+id+'" class="quick_delete_permanently_taxonomy_button btn btn-danger btn-xs">Xóa vĩnh viễn</button><button type="button" data-id="'+id+'" class="quick_public_taxonomy_button btn btn-info btn-xs">Phục hồi</button></td>';
			}
		...
		if ("first" in pagination){
			...
			var pagination_bar = '<a data-paged="'+first+'" class="btn btn-default btn-xs">Trang đầu</a>'+previous_link+'<a data-paged="'+paged+'" class="btn btn-default btn-xs custom_paged">'+paged+'</a>'+next_link+'<a data-paged="'+last+'" class="btn btn-default btn-xs">Trang cuối</a><span class="total_page">( Trang '+paged+' trong tổng số '+last+' trang, '+total+' kết quả )</span>';
	...
	$(document).on('click', '.quick_edit_taxonomy_button', function(){
		...
		$.post( '?run=taxonomy_ajax.php&key='+taxonomy_key+'&action=quick_edit&id='+id, function( form_template ) {
			$('.taxonomy_tr.taxonomy_'+id).after('<tr class="quick_edit_tr"><td colspan="4" class="quick_edit_td"><div class="quick_edit_wapper">'+form_template+'</div></td></tr>');
		});
	});
	...
	$(document).on('click', '.quick_delete_taxonomy_button', function(){
		...
		$.post( '?run=taxonomy_ajax.php&action=draft', { id:id }, function( data ) {
			...
			$('.td_action_'+id).append('<button type="button" data-id="'+id+'" class="quick_public_taxonomy_button btn btn-info btn-xs">Phục hồi</button>');
	...
	$(document).on('click', '.quick_delete_permanently_taxonomy_button', function(){
		...
		$('.td_action_'+id).append('<button type="button" data-id="'+id+'" class="confirm_delete_permanently_taxonomy_button btn btn-danger btn-xs">Xác nhận xóa vĩnh viễn</button>');
	...
	$(document).on('click', '.quick_public_taxonomy_button', function(){
		...
		$.post( '?run=taxonomy_ajax.php&action=public', { id:id }, function( data ) {
			...
			$('.td_action_'+id).append('<button type="button" data-id="'+id+'" class="quick_delete_taxonomy_button btn btn-danger btn-xs">Xóa</button>');
			$.notify('Đã phục hồi lại danh mục', { globalPosition: 'top right',className: 'success' } );
		});
	});
