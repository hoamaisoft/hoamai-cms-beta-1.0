$(document).ready(function(){
	...
	$('.submit_new_media_group').click(function(e) {
		...
		if(group_name==''){
			...
			$.post( '?run=media_ajax.php&action=add_media_group', { group_name: group_name, group_parent: group_id })
				...
				$('.media_file_show').html('Đang tải dữ liệu');
	...
	$('.use_media_file_insert').click(function(){
		...
		if(multi=='true'){
			var thisval = $('input[use_media_file='+use_media_file+']').val();
			if(thisval != ''){
				var newval = thisval+','+file_id;
				$('input[use_media_file='+use_media_file+']').val(newval);
			}else{
				$('input[use_media_file='+use_media_file+']').val(file_id);
			}
			...
			$.post( '?run=media_ajax.php&action=thumbnail_media', { id:file_id })
				...
				$('.preview_media_file[use_media_file='+use_media_file+']').append(thumb);
			});
		}
		if(multi=='false'){
			$('input[use_media_file='+use_media_file+']').val(file_id);
			...
			$.post( '?run=media_ajax.php&action=thumbnail_media', { id:file_id })
				...
				$('.preview_media_file[use_media_file='+use_media_file+']').html(thumb);
			});
		}
